...
namespace mkpp {
  ...
  struct SolverKernels {
      ...
      KOKKOS_INLINE_FUNCTION void compute_rates(const StateView& state, RateView& F_block, const double* jvals) const {
          ...
          const double R_0 = state(6)*jvals[0];
          const double R_1 = 8.0180000000000003e-17*state(0)*state(6);
          const double R_2 = state(2)*jvals[1];
          const double R_3 = 1.576e-15*state(0)*state(2);
          const double R_4 = state(2)*jvals[2];
          const double R_5 = 7.1100000000000005e-11*state(5)*state(1);
          const double R_6 = 1.2e-10*state(1)*state(2);
          const double R_7 = 6.0620000000000003e-15*state(3)*state(2);
          const double R_8 = 1.0690000000000001e-11*state(4)*state(0);
          const double R_9 = state(4)*jvals[3];
      ...
      KOKKOS_INLINE_FUNCTION void integrate(double dt_total, StateView& state, const double* jvals) const {
          ...
          const double R_1 = 8.0180000000000003e-17*S_0*S_6;
          const double R_2 = S_2*jvals[1];
          const double R_3 = 1.576e-15*S_0*S_2;
          const double R_4 = S_2*jvals[2];
          const double R_5 = 7.1100000000000005e-11*S_5*S_1;
          const double R_6 = 1.2e-10*S_1*S_2;
          const double R_7 = 6.0620000000000003e-15*S_3*S_2;
          const double R_8 = 1.0690000000000001e-11*S_4*S_0;
