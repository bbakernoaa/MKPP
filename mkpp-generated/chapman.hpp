...
struct SolverKernels {
    ...
    void compute_rates(const StateView& state, RateView& F_block) const {
        F_block(0) = -6.0e-34 * state(3) * state(0) * state(1) -
                     7.9999999999999998e-12 * state(0) * state(2) +
                     2.0000000000000002e-5 * state(1) + 0.001 * state(2);
        F_block(1) = 0.0;
        F_block(2) = 6.0e-34 * state(3) * state(0) * state(1) -
                     7.9999999999999998e-12 * state(0) * state(2) - 0.001 * state(2);
        F_block(3) = 0.0;
    }
    ...
    void compute_tlm(const StateView& state, const DeltaView& delta_C, RateView& dF_block) const {
        dF_block(0) = 0.0;
        dF_block(0) +=
            (-6.0e-34 * state(3) * state(1) - 7.9999999999999998e-12 * state(2)) * delta_C(0);
    ...
    void integrate(double dt, StateView& state) const {
        ...
        // 1. Stage 1 Rates (F1)
        double F1_0 = -6.0e-34 * state(3) * state(0) * state(1) -
                      7.9999999999999998e-12 * state(0) * state(2) +
                      2.0000000000000002e-5 * state(1) + 0.001 * state(2);
        double F1_1 = 0.0;
        double F1_2 = 6.0e-34 * state(3) * state(0) * state(1) -
                      7.9999999999999998e-12 * state(0) * state(2) - 0.001 * state(2);
        double F1_3 = 0.0;
        ...
        double F2_0 = -6.0e-34 * Ynew_3 * Ynew_0 * Ynew_1 -
                      7.9999999999999998e-12 * Ynew_0 * Ynew_2 + 2.0000000000000002e-5 * Ynew_1 +
                      0.001 * Ynew_2;
        double rhs2_0 = F2_0 + (ros_C0 / dt) * K1_0;
        double F2_1 = 0.0;
        double rhs2_1 = F2_1 + (ros_C0 / dt) * K1_1;
        double F2_2 = 6.0e-34 * Ynew_3 * Ynew_0 * Ynew_1 -
                      7.9999999999999998e-12 * Ynew_0 * Ynew_2 - 0.001 * Ynew_2;
