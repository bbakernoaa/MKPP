...
namespace mkpp {
  ...
  struct SolverKernels {
      ...
      KOKKOS_INLINE_FUNCTION void compute_rates(const StateView& state, RateView& F_block, const double* jvals) const {
          ...
          const double R_0 = state(1)*jvals[0];
          const double R_1 = 6.0e-34*state(3)*state(0)*state(1);
          const double R_2 = state(2)*jvals[1];
          const double R_3 = 7.9999999999999998e-12*state(0)*state(2);
      ...
      KOKKOS_INLINE_FUNCTION void integrate(double dt_total, StateView& state, const double* jvals) const {
          ...
          const double R_1 = 6.0e-34*S_3*S_0*S_1;
          const double R_2 = S_2*jvals[1];
          const double R_3 = 7.9999999999999998e-12*S_0*S_2;
