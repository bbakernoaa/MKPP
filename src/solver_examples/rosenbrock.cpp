...
int Rosenbrock(KPP_REAL Y[], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL AbsTol[], KPP_REAL RelTol[],
               void (*ode_Fun)(KPP_REAL, KPP_REAL[], KPP_REAL[]),
               void (*ode_Jac)(KPP_REAL, KPP_REAL[], KPP_REAL[]), KPP_REAL RPAR[], int IPAR[]);
int RosenbrockIntegrator(KPP_REAL Y[], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL AbsTol[],
                         KPP_REAL RelTol[], void (*ode_Fun)(KPP_REAL, KPP_REAL[], KPP_REAL[]),
                         void (*ode_Jac)(KPP_REAL, KPP_REAL[], KPP_REAL[]), int ros_S,
                         KPP_REAL ros_M[], KPP_REAL ros_E[], KPP_REAL ros_A[], KPP_REAL ros_C[],
                         KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], KPP_REAL ros_ELO,
                         char ros_NewF[], char Autonomous, char VectorTol, int Max_no_steps,
...
char ros_PrepareMatrix(KPP_REAL *H, int Direction, KPP_REAL gam, KPP_REAL Jac0[], KPP_REAL Ghimj[],
                       int Pivot[]);
KPP_REAL ros_ErrorNorm(KPP_REAL Y[], KPP_REAL Ynew[], KPP_REAL Yerr[], KPP_REAL AbsTol[],
                       KPP_REAL RelTol[], char VectorTol);
...
void ros_FunTimeDerivative(KPP_REAL T, KPP_REAL Roundoff, KPP_REAL Y[], KPP_REAL Fcn0[],
                           void ode_Fun(KPP_REAL, KPP_REAL[], KPP_REAL[]), KPP_REAL dFdT[]);
...
void Ros2(int *ros_S, KPP_REAL ros_A[], KPP_REAL ros_C[], KPP_REAL ros_M[], KPP_REAL ros_E[],
          KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], char ros_NewF[], KPP_REAL *ros_ELO,
          char *ros_Name);
void Ros3(int *ros_S, KPP_REAL ros_A[], KPP_REAL ros_C[], KPP_REAL ros_M[], KPP_REAL ros_E[],
          KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], char ros_NewF[], KPP_REAL *ros_ELO,
          char *ros_Name);
void Ros4(int *ros_S, KPP_REAL ros_A[], KPP_REAL ros_C[], KPP_REAL ros_M[], KPP_REAL ros_E[],
          KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], char ros_NewF[], KPP_REAL *ros_ELO,
          char *ros_Name);
void Rodas3(int *ros_S, KPP_REAL ros_A[], KPP_REAL ros_C[], KPP_REAL ros_M[], KPP_REAL ros_E[],
            KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], char ros_NewF[], KPP_REAL *ros_ELO,
            char *ros_Name);
void Rodas3_1(int *ros_S, KPP_REAL ros_A[], KPP_REAL ros_C[], KPP_REAL ros_M[], KPP_REAL ros_E[],
              KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], char ros_NewF[], KPP_REAL *ros_ELO,
              char *ros_Name);
void Rodas4(int *ros_S, KPP_REAL ros_A[], KPP_REAL ros_C[], KPP_REAL ros_M[], KPP_REAL ros_E[],
            KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], char ros_NewF[], KPP_REAL *ros_ELO,
            char *ros_Name);
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    IERR = Rosenbrock(VAR, TIN, TOUT, ATOL, RTOL, &FunTemplate, &JacTemplate, RPAR, IPAR);
...
#define Smax 6
  ...
  !   For Vector tolerances (IPAR[1] == 0) the code uses AbsTol(1:NVAR) and RelTol(1:NVAR) */
    ...
    switch (Method) {
        case 1:
            Ros2(&ros_S, ros_A, ros_C, ros_M, ros_E, ros_Alpha, ros_Gamma, ros_NewF, &ros_ELO,
                 ros_Name);
            break;
        case 2:
            Ros3(&ros_S, ros_A, ros_C, ros_M, ros_E, ros_Alpha, ros_Gamma, ros_NewF, &ros_ELO,
                 ros_Name);
            break;
        case 3:
            Ros4(&ros_S, ros_A, ros_C, ros_M, ros_E, ros_Alpha, ros_Gamma, ros_NewF, &ros_ELO,
                 ros_Name);
            break;
        case 4:
            Rodas3(&ros_S, ros_A, ros_C, ros_M, ros_E, ros_Alpha, ros_Gamma, ros_NewF, &ros_ELO,
                   ros_Name);
            break;
        case 5:
            Rodas4(&ros_S, ros_A, ros_C, ros_M, ros_E, ros_Alpha, ros_Gamma, ros_NewF, &ros_ELO,
                   ros_Name);
            break;
        case 7:
            Rodas3_1(&ros_S, ros_A, ros_C, ros_M, ros_E, ros_Alpha, ros_Gamma, ros_NewF, &ros_ELO,
                     ros_Name);
            break;
        default:
    ...
    /*~~~>  Rosenbrock method   */
    IERR = RosenbrockIntegrator(Y, Tstart, Tend, AbsTol, RelTol, ode_Fun, ode_Jac,
                                /*  Rosenbrock method coefficients  */
                                ros_S, ros_M, ros_E, ros_A, ros_C, ros_Alpha, ros_Gamma, ros_ELO,
                                ros_NewF,
                                /*  Integration parameters */
                                Autonomous, VectorTol, Max_no_steps, Roundoff, Hmin, Hmax, Hstart,
...
int RosenbrockIntegrator(
    ...
    int ros_S, KPP_REAL ros_M[], KPP_REAL ros_E[], KPP_REAL ros_A[], KPP_REAL ros_C[],
    KPP_REAL ros_Alpha[], KPP_REAL ros_Gamma[], KPP_REAL ros_ELO, char ros_NewF[],
    /*~~~> Input: integration parameters  */
    char Autonomous, char VectorTol, int Max_no_steps, KPP_REAL Roundoff, KPP_REAL Hmin,
    KPP_REAL Hmax, KPP_REAL Hstart, KPP_REAL FacMin, KPP_REAL FacMax, KPP_REAL FacRej,
    KPP_REAL FacSafe,
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    KPP_REAL Ynew[KPP_NVAR], Fcn0[KPP_NVAR], Fcn[KPP_NVAR], dFdT[KPP_NVAR], Jac0[KPP_LU_NONZERO],
        Ghimj[KPP_LU_NONZERO];
    KPP_REAL K[KPP_NVAR * 6];
    ...
    std::experimental::mdspan<KPP_REAL, std::experimental::extents<size_t, std::dynamic_extent>>
        a_view(ros_A, ros_S * (ros_S - 1) / 2);
    std::experimental::mdspan<KPP_REAL, std::experimental::extents<size_t, std::dynamic_extent>>
        c_view(ros_C, ros_S * (ros_S - 1) / 2);
    std::experimental::mdspan<
        KPP_REAL, std::experimental::extents<size_t, std::dynamic_extent, std::dynamic_extent>>
        k_view(K, ros_S, KPP_NVAR);
    ...
    *Hexit = 0.0;
    ...
    while (((Direction > 0) && ((T - Tend) + Roundoff <= ZERO)) ||
        ...
        *Hexit = H;
        ...
        if (!Autonomous) ros_FunTimeDerivative(T, Roundoff, Y, Fcn0, ode_Fun, dFdT);
        ...
        while (1) { /* WHILE STEP NOT ACCEPTED */
            ...
            if (ros_PrepareMatrix(&H, Direction, ros_Gamma[0], Jac0, Ghimj,
                                  Pivot)) { /* More than 5 consecutive failed decompositions */
                *Texit = T;
            ...
            for (istage = 1; istage <= ros_S; istage++) {
                ...
                if (istage == 1)
                    ...
                    if (ros_NewF[istage - 1]) {
                        ...
                        for (j = 1; j <= istage - 1; j++)
                            WAXPY(KPP_NVAR, a_view((istage - 1) * (istage - 2) / 2 + j - 1),
                                  (KPP_REAL *)&k_view(j - 1, 0), 1, Ynew, 1);
                ...
                for (j = 1; j <= istage - 1; j++) {
                    HC = c_view((istage - 1) * (istage - 2) / 2 + j - 1) / (Direction * H);
                    WAXPY(KPP_NVAR, HC, (KPP_REAL *)&k_view(j - 1, 0), 1, &K[ioffset], 1);
                } /* for j */
...
KPP_REAL ros_ErrorNorm(
    /*~~~> Input arguments */
    KPP_REAL Y[], KPP_REAL Ynew[], KPP_REAL Yerr[], KPP_REAL AbsTol[], KPP_REAL RelTol[],
    char VectorTol)
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
void ros_FunTimeDerivative(
    /*~~~> Input arguments: */
    KPP_REAL T, KPP_REAL Roundoff, KPP_REAL Y[], KPP_REAL Fcn0[],
    void (*ode_Fun)(KPP_REAL, KPP_REAL[], KPP_REAL[]),
    /*~~~> Output arguments: */
    KPP_REAL dFdT[])
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
