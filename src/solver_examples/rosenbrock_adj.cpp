...
KPP_REAL ros_A[15], ros_C[15], ros_M[6], ros_E[6], ros_Alpha[6], ros_Gamma[6], ros_ELO;
int ros_NewF[6];   /* Holds Boolean values */
char ros_Name[12]; /* Length 12 */

/*~~~>  Types of Adjoints Implemented */
enum adjoint { Adj_none = 1, Adj_discrete = 2, Adj_continuous = 3, Adj_simple_continuous = 4 };
...
/* Function Headers */
void INTEGRATE_ADJ(int NADJ, KPP_REAL Y[], KPP_REAL Lambda[][NVAR], KPP_REAL TIN, KPP_REAL TOUT,
                   KPP_REAL ATOL_adj[][NVAR], KPP_REAL RTOL_adj[][NVAR], int ICNTRL_U[],
                   KPP_REAL RCNTRL_U[], int ISTATUS_U[], KPP_REAL RSTATUS_U[]);
int RosenbrockADJ(KPP_REAL Y[], int NADJ, KPP_REAL Lambda[][NVAR], KPP_REAL Tstart, KPP_REAL Tend,
                  KPP_REAL AbsTol[], KPP_REAL RelTol[], KPP_REAL AbsTol_adj[][NVAR],
                  KPP_REAL RelTol_adj[][NVAR], KPP_REAL RCNTRL[], int ICNTRL[], KPP_REAL RSTATUS[],
                  int ISTATUS[]);
void ros_AllocateDBuffers(int S, int SaveLU);
void ros_FreeDBuffers(int SaveLU);
void ros_AllocateCBuffers();
void ros_FreeCBuffers();
void ros_DPush(int S, KPP_REAL T, KPP_REAL H, KPP_REAL Ystage[], KPP_REAL K[], KPP_REAL E[],
               int P[], int SaveLU);
void ros_DPop(int S, KPP_REAL* T, KPP_REAL* H, KPP_REAL* Ystage, KPP_REAL* K, KPP_REAL* E, int* P,
              int SaveLU);
void ros_CPush(KPP_REAL T, KPP_REAL H, KPP_REAL Y[], KPP_REAL dY[], KPP_REAL d2Y[]);
void ros_CPop(KPP_REAL T, KPP_REAL H, KPP_REAL Y[], KPP_REAL dY[], KPP_REAL d2Y[]);
int ros_ErrorMsg(int Code, KPP_REAL T, KPP_REAL H);
int ros_FwdInt(KPP_REAL Y[], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL T, KPP_REAL AbsTol[],
               KPP_REAL RelTol[], int AdjointType, KPP_REAL Hmin, KPP_REAL Hstart, KPP_REAL Hmax,
               KPP_REAL Roundoff, int ISTATUS[], int Max_no_steps, KPP_REAL RSTATUS[],
               int Autonomous, int VectorTol, KPP_REAL FacMax, KPP_REAL FacMin, KPP_REAL FacSafe,
               KPP_REAL FacRej, int SaveLU);
int ros_DadjInt(int NADJ, KPP_REAL Lambda[][NVAR], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL T,
                int SaveLU, int ISTATUS[], KPP_REAL Roundoff, int Autonomous);
int ros_CadjInt(int NADJ, KPP_REAL Y[][NVAR], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL T,
                KPP_REAL AbsTol_adj[][NVAR], KPP_REAL RelTol_adj[][NVAR], KPP_REAL RSTATUS[],
                KPP_REAL Hmin, KPP_REAL Hmax, KPP_REAL Hstart, KPP_REAL Roundoff, int Max_no_steps,
                int Autonomous, int VectorTol, KPP_REAL FacMax, KPP_REAL FacMin, KPP_REAL FacSafe,
                KPP_REAL FacRej, int ISTATUS[]);
int ros_SimpleCadjInt(int NADJ, KPP_REAL Y[][NVAR], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL T,
                      int ISTATUS[], int Autonomous, KPP_REAL Roundoff);
KPP_REAL ros_ErrorNorm(KPP_REAL Y[], KPP_REAL Ynew[], KPP_REAL Yerr[], KPP_REAL AbsTol[],
                       KPP_REAL RelTol[], int VectorTol);
void ros_FunTimeDerivative(KPP_REAL T, KPP_REAL Roundoff, KPP_REAL Y[], KPP_REAL Fcn0[],
                           KPP_REAL dFdT[], int ISTATUS[]);
void ros_JacTimeDerivative(KPP_REAL T, KPP_REAL Roundoff, KPP_REAL Y[], KPP_REAL Jac0[],
                           KPP_REAL dJdT[], int ISTATUS[]);
int ros_PrepareMatrix(KPP_REAL H, int Direction, KPP_REAL gam, KPP_REAL Jac0[], KPP_REAL Ghimj[],
                      int Pivot[], int ISTATUS[]);
void ros_Decomp(KPP_REAL A[], int Pivot[], int* ising, int ISTATUS[]);
void ros_Solve(char How, KPP_REAL A[], int Pivot[], KPP_REAL b[], int ISTATUS[]);
void ros_cadj_Y(KPP_REAL T, KPP_REAL Y[]);
void ros_Hermite3(KPP_REAL a, KPP_REAL b, KPP_REAL T, KPP_REAL Ya[], KPP_REAL Yb[], KPP_REAL Ja[],
                  KPP_REAL Jb[], KPP_REAL Y[]);
void ros_Hermite5(KPP_REAL a, KPP_REAL b, KPP_REAL T, KPP_REAL Ya[], KPP_REAL Yb[], KPP_REAL Ja[],
                  KPP_REAL Jb[], KPP_REAL Ha[], KPP_REAL Hb[], KPP_REAL Y[]);
void Ros2();
void Ros3();
void Ros4();
void Rodas3();
...
void INTEGRATE_ADJ(int NADJ, KPP_REAL Y[], KPP_REAL Lambda[][NVAR], KPP_REAL TIN, KPP_REAL TOUT,
    ...
    IERR = RosenbrockADJ(Y, NADJ, Lambda, TIN, TOUT, ATOL, RTOL, ATOL_adj, RTOL_adj, RCNTRL, ICNTRL,
                         RSTATUS, ISTATUS);

    if (IERR < 0) printf("RosenbrockADJ: Unsucessful step at T=%f (IERR=%d)", TIN, IERR);
...
int RosenbrockADJ(KPP_REAL Y[], int NADJ, KPP_REAL Lambda[][NVAR], KPP_REAL Tstart, KPP_REAL Tend,
    ...
    } else if ((AdjointType == Adj_continuous) || (AdjointType == Adj_simple_continuous)) {
        ros_AllocateCBuffers();
    }

    /*~~~>  CALL Forward Rosenbrock method */
    IERR = ros_FwdInt(Y, Tstart, Tend, Texit, AbsTol, RelTol, AdjointType, Hmin, Hstart, Hmax,
                      Roundoff, ISTATUS, Max_no_steps, RSTATUS, Autonomous, VectorTol, FacMax,
                      FacMin, FacSafe, FacRej, SaveLU);
    ...
    if ((AdjointType == Adj_continuous) || (AdjointType == Adj_simple_continuous)) {
        switch (CadjMethod) {
            case 1:
                Ros2();
                break;
    ...
    switch (AdjointType) {
        case Adj_discrete:
            IERR = ros_DadjInt(NADJ, Lambda, Tstart, Tend, Texit, SaveLU, ISTATUS, Roundoff,
                               Autonomous);
            break;
        case Adj_continuous:
            IERR = ros_CadjInt(NADJ, Lambda, Tend, Tstart, Texit, AbsTol_adj, RelTol_adj, RSTATUS,
                               Hmin, Hmax, Hstart, Roundoff, Max_no_steps, Autonomous, VectorTol,
                               FacMax, FacMin, FacSafe, FacRej, ISTATUS);
            break;
        case Adj_simple_continuous:
            IERR =
                ros_SimpleCadjInt(NADJ, Lambda, Tstart, Tend, Texit, ISTATUS, Autonomous, Roundoff);
...
void ros_FreeDBuffers(int SaveLU) {
    ...
    if (SaveLU) {
        for (i = 0; i < bufsize; i++) free(chk_J[i]);
        free(chk_J);
    }
...
void ros_DPush(int S, KPP_REAL T, KPP_REAL H, KPP_REAL Ystage[], KPP_REAL K[], KPP_REAL E[],
    ...
    if (SaveLU) {
        ...
        int j;
        for (j = 0; j < NVAR; j++) {
            for (i = 0; i < NVAR; i++) chk_J[stack_ptr][i][j] = E[i][j];
            chk_P[stack_ptr][j] = P[j];
        }
#else
        for (i = 0; i < LU_NONZERO; i++) chk_J[stack_ptr][i] = E[i];
#endif
    }
...
void ros_DPop(int S, KPP_REAL* T, KPP_REAL* H, KPP_REAL* Ystage, KPP_REAL* K, KPP_REAL* E, int* P,
    ...
    *T = chk_T[stack_ptr];
    ...
    if (SaveLU) {
        ...
        int j;
        for (i = 0; i < NVAR; i++) {
            for (j = 0; j < NVAR; j++) E[(j * NVAR) + i] = chk_J[stack_ptr][j][i];
            P[i] = chk_P[stack_ptr][i];
        }
#else
        for (i = 0; i < LU_NONZERO; i++) E[i] = chk_J[stack_ptr][i];
#endif
    }
...
int ros_FwdInt(KPP_REAL Y[], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL T, KPP_REAL AbsTol[],
    ...
    if (ABS(H) <= ((KPP_REAL)10.0) * Roundoff) H = DeltaMin;
    ...
    while (((Direction > 0) && ((T - Tend) + Roundoff <= ZERO)) ||
        ...
        if (!Autonomous) ros_FunTimeDerivative(T, Roundoff, Y, Fcn0, dFdT, ISTATUS);
        ...
        do { /* UntilAccepted */
            ...
            Singular = ros_PrepareMatrix(H, Direction, ros_Gamma[0], Jac0, Ghimj, Pivot, ISTATUS);
            ...
            for (istage = 0; istage < ros_S; istage++) { /* Stage */
                ...
                /* istage>0 and a new function evaluation is needed at the
                   current istage */
                    ...
                    WCOPY(NVAR, Y, 1, Ynew, 1);
                    for (j = 0; j < istage; j++) {
                        WAXPY(NVAR, ros_A[(istage) * (istage - 1) / 2 + j], &K[NVAR * j], 1, Ynew,
                              1);
                    }
                ...
                if ((istage > 0) && (AdjointType == Adj_discrete)) {
                    for (i = 0; i < NVAR; i++) Ystage[ioffset + i] = Ynew[i];
                }
            ...
            WCOPY(NVAR, Y, 1, Ynew, 1);
            for (j = 0; j < ros_S; j++) WAXPY(NVAR, ros_M[j], &K[NVAR * j], 1, Ynew, 1);

            /*~~~>  Compute the error estimation */
            WSCAL(NVAR, ZERO, Yerr, 1);
            for (j = 0; j < ros_S; j++) WAXPY(NVAR, ros_E[j], &K[NVAR * j], 1, Yerr, 1);
            ...
            if ((Err <= ONE) || (H <= Hmin)) { /*~~~> Accept step */
...
#endif
                    if (!Autonomous) WAXPY(NVAR, ONE, dFdT, 1, &K[0], 1);
                ...
                RejectLastH = TRUE;
                H = Hnew;
                if (ISTATUS[Nacc] >= 1) ISTATUS[Nrej]++;
            } /* End if else - Err <= 1 */
...
#ifdef FULL_ALGEBRA
            for (j = 0; j < NVAR; j++) {
                for (i = 0; i < NVAR; i++) Ghimj[i][j] = -Ghimj[i][j];
            }
            for (i = 0; i < NVAR; i++) Ghimj[i][i] = Ghimj[i][i] + Tau;
#else
            WSCAL(LU_NONZERO, (-ONE), Ghimj, 1);
            for (i = 0; i < NVAR; i++) Ghimj[LU_DIAG[i]] = Ghimj[LU_DIAG[i]] + Tau;
#endif
...
int ros_CadjInt(int NADJ, KPP_REAL Y[][NVAR], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL T,
...
#ifdef FULL_ALGEBRA
    KPP_REAL Jac0[NVAR][NVAR], Ghimj[NVAR][NVAR], Jac[NVAR][NVAR], dJdT[NVAR][NVAR];
#else
    KPP_REAL Jac0[LU_NONZERO], Ghimj[LU_NONZERO], Jac[LU_NONZERO], dJdT[LU_NONZERO];
#endif
    KPP_REAL H, Hnew, HC, HG, Fac, Tau;
    ...
    while (((Direction > 0) && ((T - Tend) + Roundoff <= ZERO)) ||
        ...
        /* Step size too small */
        if (((T + ((KPP_REAL)0.1) * H) == T) || (H <= Roundoff)) return ros_ErrorMsg(-7, T, H);
...
#ifdef FULL_ALGEBRA
                for (i = 0; i < NVAR; i++) dFdT[iadj][i] = MATMUL(TRANSPOSE(dJdT), Y[iadj][i]);
#else
...
/*~~~>  Ydot = -J^T*Y */
        ...
        int i;
        for (i = 0; i < NVAR; i++) {
            for (j = 0; j < NVAR; j++) Jac0[i][j] = -Jac0[i][j];
        }
#else
...
#ifdef FULL_ALGEBRA
            int i;
            for (i = 0; i < NVAR; i++) Fcn0[iadj][i] = MATMUL(TRANSPOSE(Jac0), Y[iadj][i]);
#else
        ...
        do { /* UntilAccepted */
...
#ifdef FULL_ALGEBRA
                    for (i = 0; i < NVAR; i++) {
                        for (j = 0; j < NVAR; j++) Jac[i][j] = -Jac[i][j];
                    }
#else
            ...
            iadj = 1;
            Err = ros_ErrorNorm(&Y[iadj][0], &Ynew[iadj][0], &Yerr[iadj][0], &AbsTol_adj[iadj][0],
                                &RelTol_adj[iadj][0], VectorTol);
...
int ros_SimpleCadjInt(int NADJ, KPP_REAL Y[][NVAR], KPP_REAL Tstart, KPP_REAL Tend, KPP_REAL T,
    ...
    for (istack = stack_ptr; istack >= 1; istack--) { /* TimeLoop */
        T = chk_T[istack];
        H = chk_H[istack - 1];
        for (i = 0; i < NVAR; i++) Y0[i] = chk_Y[istack][i];
...
/*~~~>  Ydot = -J^T*Y */
...
#ifdef FULL_ALGEBRA
        for (i = 0; i < NVAR; i++) {
            for (j = 0; j < NVAR; j++) Ghimj[i][j] = -Jac0[i][j];
        }
        for (i = 0; i < NVAR; i++) Ghimj[i][i] = Ghimj[i][i] + ghinv;
#else
        WCOPY(LU_NONZERO, Jac0, 1, Ghimj, 1);
        WSCAL(LU_NONZERO, (-ONE), Ghimj, 1);
        for (i = 0; i < NVAR; i++) Ghimj[LU_DIAG[i]] = Ghimj[LU_DIAG[i]] + ghinv;
#endif
        ...
        for (istage = 0; istage < ros_S; istage++) { /* Stage */
           ...
           at the current istage */
                ...
                Tau = T + ros_Alpha[istage] * Direction * H;
                for (i = 0; i < NVAR; i++)
                    ros_Hermite3(chk_T[istack - 1], chk_T[istack], Tau, &chk_Y[istack - 1][i],
                                 &chk_Y[istack][i], &chk_dY[istack - 1][i], &chk_dY[istack][i], Y0);
                JacTemplate(Tau, Y0, Jac);
                ISTATUS[Njac]++;
                ...
                for (iadj = 0; iadj < NADJ; iadj++) {
#ifdef FULL_ALGEBRA
                    for (i = 0; i < NVAR; i++) Fcn[iadj][i] = MATMUL(TRANSPOSE(Jac), Ynew[iadj][i]);
#else
                    JacTR_SP_Vec(Jac, &Ynew[iadj][0], &Fcn[iadj][0]);
#endif
                }
            ...
            for (iadj = 0; iadj < NADJ; iadj++) WCOPY(NVAR, &Fcn[iadj][0], 1, &K[iadj][ioffset], 1);
            for (j = 0; j < istage - 1; j++) {
                HC = ros_C[istage * (istage - 1) / 2 + j] / (Direction * H);
                for (iadj = 0; iadj < NADJ; iadj++)
...
void ros_cadj_Y(KPP_REAL T, KPP_REAL Y[]) {
    ...
    for (i = 0; i < stack_ptr - 1; i++) {
        if ((T >= chk_T[i]) && (T <= chk_T[i + 1])) exit(0);
    }

    for (j = 0; j < NVAR; j++)
        ros_Hermite3(chk_T[i], chk_T[i + 1], T, &chk_Y[i][j], &chk_Y[i + 1][j], &chk_dY[i][j],
                     &chk_dY[i + 1][j], Y);
...
void ros_Hermite3(KPP_REAL a, KPP_REAL b, KPP_REAL T, KPP_REAL Ya[], KPP_REAL Yb[], KPP_REAL Ja[],
    ...
    for (i = 1; i < 3; i++) amb[i] = amb[i - 1] * amb[0];
    ...
    for (j = 2; j >= 0; j--) WAXPY(NVAR, pow(Tau, (j - 1)), &C[j][0], 1, Y, 1);
...
void ros_Hermite5(KPP_REAL a, KPP_REAL b, KPP_REAL T, KPP_REAL Ya[], KPP_REAL Yb[], KPP_REAL Ja[],
    ...
    amb[0] = ((KPP_REAL)1.0) / (a - b);
    for (i = 1; i < 5; i++) amb[i] = amb[i - 1] * amb[0];
...
void JacTemplate(KPP_REAL T, KPP_REAL Y[], KPP_REAL Jcb[]) {
    ...
    for (j = 0; j < NVAR; j++) {
        for (i = 0; i < NVAR; i++) Jcb[i][j] = (KPP_REAL)0.0;
    }
    for (i = 0; i < LU_NONZERO; i++) Jcb[LU_ICOL[i]][LU_IROW[i]] = JV[i];
#else
