...
int rkMethod, SdirkError;
KPP_REAL rkT[RKmax][RKmax], rkTinv[RKmax][RKmax], rkTinvAinv[RKmax][RKmax], rkAinvT[RKmax][RKmax],
    rkA[RKmax + 1][RKmax + 1], rkB[RKmax + 1], rkC[RKmax + 1], rkD[RKmax + 1], rkE[RKmax + 1],
    rkBgam[RKmax + 2], rkBhat[RKmax + 2], rkTheta[RKmax + 1], rkF[RKmax + 2], rkGamma, rkAlpha,
    rkBeta, rkELO;
/*~~~> Function headers */
...
void RungeKutta(int N, KPP_REAL T, KPP_REAL Tend, KPP_REAL Y[], KPP_REAL RelTol[],
                KPP_REAL AbsTol[], KPP_REAL RCNTRL[], int ICNTRL[], KPP_REAL RSTATUS[],
                int ISTATUS[], int* IERR);
void RK_Integrator(int N, KPP_REAL* T, KPP_REAL Tend, KPP_REAL Y[], KPP_REAL AbsTol[],
                   KPP_REAL RelTol[], int ITOL, int ISTATUS[], KPP_REAL RSTATUS[], KPP_REAL Hmin,
                   KPP_REAL Hmax, KPP_REAL Hstart, KPP_REAL Roundoff, int Max_no_steps,
                   int NewtonMaxit, int StartNewton, int Gustafsson, KPP_REAL ThetaMin,
...
void RK_ErrorScale(int N, int ITOL, KPP_REAL AbsTol[], KPP_REAL RelTol[], KPP_REAL Y[],
                   KPP_REAL SCAL[]);
/*void RK_Transform(int N, KPP_REAL Tr[][RKmax], KPP_REAL Z1[], KPP_REAL Z2[],
               KPP_REAL Z3[], KPP_REAL W1[], KPP_REAL W2[], KPP_REAL W3[]);*/
void RK_Interpolate(char action[], int N, KPP_REAL H, KPP_REAL Hold, KPP_REAL Z1[], KPP_REAL Z2[],
                    KPP_REAL Z3[], KPP_REAL CONT[][RKmax]);
void RK_PrepareRHS(int N, KPP_REAL T, KPP_REAL H, KPP_REAL Y[], KPP_REAL FO[], KPP_REAL Z1[],
                   KPP_REAL Z2[], KPP_REAL Z3[], KPP_REAL R1[], KPP_REAL R2[], KPP_REAL R3[]);
void RK_Decomp(int N, KPP_REAL H, KPP_REAL FJAC[], KPP_REAL E1[], int IP1[], KPP_REAL E2R[],
               KPP_REAL E2I[], int IP2[], int* ISING, int ISTATUS[]);
void RK_Solve(int N, KPP_REAL H, KPP_REAL E1[], int IP1[], KPP_REAL E2R[], KPP_REAL E2I[],
              int IP2[], KPP_REAL R1[], KPP_REAL R2[], KPP_REAL R3[], int ISTATUS[]);
void RK_ErrorEstimate(int N, KPP_REAL H, KPP_REAL T, KPP_REAL Y[], KPP_REAL FO[], KPP_REAL E1[],
                      int IP1[], KPP_REAL Z1[], KPP_REAL Z2[], KPP_REAL Z3[], KPP_REAL SCAL[],
                      KPP_REAL* Err, int FirstStep, int Reject, int ISTATUS[]);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
{
    ...
    int Max_no_steps;
    KPP_REAL Hmin, Hmax, Hstart, Qmin, Qmax, Roundoff, ThetaMin, NewtonTol, FacSafe, FacMin, FacMax,
        FacRej;
    /*~~~> Local variables */
    int NewtonMaxit, ITOL, i, StartNewton, Gustafsson;
    ...
    *IERR = 0;
    ...
    switch (ICNTRL[2]) {
        case 0:
        case 1:
            Radau2A_Coefficients(); /*printf("case 0 or 1\n");*/
            break;
        case 2:
    ...
    RK_Integrator(N, &T, Tend, Y, AbsTol, RelTol, ITOL, ISTATUS, RSTATUS, Hmin, Hmax, Hstart,
                  Roundoff, Max_no_steps, NewtonMaxit, StartNewton, Gustafsson, ThetaMin, NewtonTol,
...
void RK_Integrator(int N,
                   ...
                   KPP_REAL* T, KPP_REAL Tend, KPP_REAL Y[], KPP_REAL AbsTol[], KPP_REAL RelTol[],
                   int ITOL,
                   /*~~~> Input: the initial condition at T; output: the solution at Tend */
                   int ISTATUS[], KPP_REAL RSTATUS[], KPP_REAL Hmin, KPP_REAL Hmax, KPP_REAL Hstart,
                   KPP_REAL Roundoff, int Max_no_steps, int NewtonMaxit, int StartNewton,
                   int Gustafsson, KPP_REAL ThetaMin, KPP_REAL NewtonTol, KPP_REAL FacSafe,
                   KPP_REAL FacMax, KPP_REAL FacMin, KPP_REAL FacRej, KPP_REAL Qmin, KPP_REAL Qmax,
                   int* IERR) {
    KPP_REAL FJAC[LU_NONZERO], E1[LU_NONZERO], E2R[LU_NONZERO], E2I[LU_NONZERO];
    KPP_REAL Z1[NVAR], Z2[NVAR], Z3[NVAR], Z4[NVAR], SCAL[NVAR], DZ1[NVAR], DZ2[NVAR], DZ3[NVAR],
        DZ4[NVAR], G[NVAR], TMP[NVAR], FO[NVAR];
    KPP_REAL CONT[NVAR][RKmax], Tdirection, H, Hacc, Hnew, Hold, Fac, FacGus, Theta, Err, ErrOld,
        NewtonRate, NewtonIncrement, Hratio, Qnewton, NewtonPredictedErr, NewtonIncrementOld,
        ThetaSD;
    int IP1[NVAR], IP2[NVAR], NewtonIter,
        NewtonIterMain, /* BUG 4 FIX: separate variable to save main loop count */
        Nconsecutive, i;
    int ISING;
    int Reject, FirstStep, SkipJac, NewtonDone, SkipLU;
    ...
    if (ABS(H) <= ((KPP_REAL)10.0 * Roundoff)) { H = (KPP_REAL)(1.0e-06); } /* end if */
    H = SIGN(H, Tdirection);
    Hold = H;
    Reject = 0;
    FirstStep = 1;
    SkipJac = 0;
    SkipLU = 0;
    if ((*T + H * ((KPP_REAL)1.0001) - Tend) * Tdirection >= ZERO) { H = Tend - *T; } /* end if */
    Nconsecutive = 0;
    ...
    /*~~~> Time loop begins */
Tloop:
    while ((Tend - *T) * Tdirection - Roundoff > ZERO) {
        FUN_CHEM(*T, Y, FO);
        ISTATUS[Nfun]++;
        ...
        if (SkipLU == 0) {
            ...
            if (ISING != 0) {
                ISTATUS[Nsng]++;
                Nconsecutive++;
                if (Nconsecutive >= 5) { RK_ErrorMsg(-12, *T, H, IERR); }
                H = H * ((KPP_REAL)0.5);
                Reject = 1;
                SkipJac = 1;
                SkipLU = 0;
        ...
        if (((KPP_REAL)0.1) * ABS(H) <= ABS(*T) * Roundoff) RK_ErrorMsg(-10, *T, H, IERR);
        ...
        for (NewtonIter = 1; NewtonIter <= NewtonMaxit; NewtonIter++) {
            ...
            NewtonIncrement = SQRT((RK_ErrorNorm(N, SCAL, DZ1) * RK_ErrorNorm(N, SCAL, DZ1) +
            ...
            if (NewtonIter == 1) {
                ...
                if (NewtonIter < NewtonMaxit) {
                    NewtonPredictedErr =
                        NewtonIncrement * pow(Theta, (NewtonMaxit - NewtonIter)) / (ONE - Theta);
            ...
            if (NewtonDone == 1) break; /* Exit NewtonLoop */
            if (NewtonIter == NewtonMaxit) {
        ...
        if (SdirkError == 1) {
            ...
            for (i = 0; i < N; i++) { Z4[i] = Z3[i]; }
            /*~~~> Prepare the loop-independent part of the right-hand side */
            Set2Zero(N, G);
            if (rkMethod != L3A) WAXPY(N, rkBgam[0] * H, FO, 1, G, 1);
            ...
            for (NewtonIter = 1; NewtonIter <= NewtonMaxit; NewtonIter++) {
                ...
                if (NewtonIter == 1) {
                    ...
                    if (ThetaSD < (KPP_REAL)0.99) {
                        ...
                        NewtonPredictedErr = NewtonIncrement *
                                             pow(ThetaSD, (NewtonMaxit - NewtonIter)) /
                                             (ONE - ThetaSD);
                        if (NewtonPredictedErr >= NewtonTol) {
                            Qnewton = MIN((KPP_REAL)10.0, NewtonPredictedErr / NewtonTol);
                            Fac = (KPP_REAL)0.8 *
                                  pow(Qnewton, (-ONE / (1 + NewtonMaxit - NewtonIter)));
                ...
                NewtonDone = (NewtonRate * NewtonIncrement <= NewtonTol);
                if (NewtonDone == 1) break; /* EXIT SDNewtonLoop */
        ...
        if (SdirkError == 1) {
            Set2Zero(N, DZ4);
            if (rkMethod == L3A) {
                for (i = 0; i < N; i++) DZ4[i] = H * rkF[0] * FO[i];
                if (rkF[0] != ZERO) WAXPY(N, rkF[0], Z1, 1, DZ4, 1);
                if (rkF[1] != ZERO) WAXPY(N, rkF[1], Z2, 1, DZ4, 1);
                if (rkF[2] != ZERO) WAXPY(N, rkF[2], Z3, 1, DZ4, 1);
                for (i = 0; i < N; i++) TMP[i] = Y[i] + Z4[i];
                FUN_CHEM(*T + H, TMP, DZ1);
                WAXPY(N, H * rkBgam[4], DZ1, 1, DZ4, 1);
            } else {
                if (rkD[0] != ZERO) WAXPY(N, rkD[0], Z1, 1, DZ4, 1);
                if (rkD[1] != ZERO) WAXPY(N, rkD[1], Z2, 1, DZ4, 1);
                if (rkD[2] != ZERO) WAXPY(N, rkD[2], Z3, 1, DZ4, 1);
                WAXPY(N, -ONE, Z4, 1, DZ4, 1);
            } /* end if */
            Err = RK_ErrorNorm(N, SCAL, DZ4);
        } else {
            RK_ErrorEstimate(N, H, *T, Y, FO, E1, IP1, Z1, Z2, Z3, SCAL, &Err, FirstStep, Reject,
                             ISTATUS);
        } /* end if */
        ...
        if (Err < ONE) { /*~~~> STEP IS ACCEPTED */
            ...
            Hold = H;
            *T = *T + H;
            if (rkD[0] != ZERO) WAXPY(N, rkD[0], Z1, 1, Y, 1);
            if (rkD[1] != ZERO) WAXPY(N, rkD[1], Z2, 1, Y, 1);
            if (rkD[2] != ZERO) WAXPY(N, rkD[2], Z3, 1, Y, 1);
            if (StartNewton == 1) RK_Interpolate("make", N, H, Hold, Z1, Z2, Z3, CONT);
            RK_ErrorScale(N, ITOL, AbsTol, RelTol, Y, SCAL);
            RSTATUS[Ntexit] = *T;
            RSTATUS[Nhnew] = Hnew;
            RSTATUS[Nhacc] = H;
            Hnew = Tdirection * MIN(MAX(ABS(Hnew), Hmin), Hmax);
            if (Reject == 1) Hnew = Tdirection * MIN(ABS(Hnew), ABS(H));
            Reject = 0;
...
void RK_Interpolate(char action[], int N, KPP_REAL H, KPP_REAL Hold, KPP_REAL Z1[], KPP_REAL Z2[],
    ...
    if (strncmp(action, "make", 4) == 0) {
        ...
        for (i = 0; i < N; i++) {
            CONT[i][0] = (-rkC[2] * rkC[2] * rkC[1] * Z1[i] + Z3[i] * rkC[1] * rkC[0] * rkC[0] +
                          rkC[1] * rkC[1] * rkC[2] * Z1[i] - rkC[1] * rkC[1] * rkC[0] * Z3[i] +
                          rkC[2] * rkC[2] * rkC[0] * Z2[i] - Z2[i] * rkC[2] * rkC[0] * rkC[0]) /
                             den -
                         Z3[i];
            CONT[i][1] = -(rkC[0] * rkC[0] * (Z3[i] - Z2[i]) + rkC[1] * rkC[1] * (Z1[i] - Z3[i]) +
                           rkC[2] * rkC[2] * (Z2[i] - Z1[i])) /
                         den;
            CONT[i][2] =
                (rkC[0] * (Z3[i] - Z2[i]) + rkC[1] * (Z1[i] - Z3[i]) + rkC[2] * (Z2[i] - Z1[i])) /
                den;
        } /* end for loop */
    }
...
void RK_Decomp(int N, KPP_REAL H, KPP_REAL FJAC[], KPP_REAL E1[], int IP1[], KPP_REAL E2R[],
    ...
    for (i = 0; i < LU_NONZERO; i++) { E1[i] = -FJAC[i]; }
    for (i = 0; i < NVAR; i++) {
        j = LU_DIAG[i];
        E1[j] = E1[j] + Gamma;
    }
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    KppSolve(E1, TMP);
    if ((rkMethod == R1A) || (rkMethod == GAU) || (rkMethod == L3A)) KppSolve(E1, TMP);
    if (rkMethod == GAU) KppSolve(E1, TMP);
    /*~~~> Note: for a full matrix use Lapack:
    ...
    *Err = RK_ErrorNorm(N, SCAL, TMP);
    ...
    /* firej */
    if ((FirstStep == 1) || (Reject == 1)) {
        for (i = 0; i < N; i++) { TMP[i] = Y[i] + TMP[i]; } /* end for */
        FUN_CHEM(T, TMP, F1);
        ISTATUS[Nfun]++;
        for (i = 0; i < N; i++) { TMP[i] = F1[i] + F2[i]; } /* end for */
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    for (i = 0; i < N; i++) { RK_ErrorNorm = RK_ErrorNorm + (DY[i] * SCAL[i]) * (DY[i] * SCAL[i]); }
