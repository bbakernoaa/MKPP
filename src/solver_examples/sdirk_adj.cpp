...
KPP_REAL rkGamma, rkA[5][5], rkB[5], rkC[5], rkD[5], rkE[5], rkBhat[5], rkELO, rkAlpha[5][5],
    rkTheta[5][5];
...
int SDIRKADJ(int N, int NADJ, KPP_REAL Tinitial, KPP_REAL Tfinal, KPP_REAL Y[],
             ...
             KPP_REAL RelTol_adj[][NVAR], KPP_REAL AbsTol_adj[][NVAR], KPP_REAL RCNTRL[],
             int ICNTRL[], KPP_REAL RSTATUS[], int ISTATUS[]);
int SDIRK_FwdInt(int N, KPP_REAL Tinitial, KPP_REAL Tfinal, KPP_REAL Y[], KPP_REAL Hmax,
                 KPP_REAL Hmin, KPP_REAL Hstart, KPP_REAL Roundoff, KPP_REAL AbsTol[],
                 KPP_REAL RelTol[], int ISTATUS[], KPP_REAL RSTATUS[], int Max_no_steps,
                 int NewtonMaxit, KPP_REAL NewtonTol, KPP_REAL ThetaMin, KPP_REAL FacSafe,
                 KPP_REAL FacMin, KPP_REAL FacMax, KPP_REAL FacRej, KPP_REAL Qmin, KPP_REAL Qmax,
                 int ITOL, int SaveLU);
int SDIRK_DadjInt(int N, int NADJ, KPP_REAL Lambda[][NVAR], int SaveLU, int ISTATUS[], int ITOL,
                  KPP_REAL AbsTol_adj[][NVAR], KPP_REAL RelTol_adj[][NVAR], int NewtonMaxit,
...
void SDIRK_Push(KPP_REAL T, KPP_REAL H, KPP_REAL Y[], KPP_REAL Z[][NVAR], KPP_REAL E[], int P[],
                int Max_no_steps, int SaveLU);
void SDIRK_Pop(KPP_REAL* T, KPP_REAL* H, KPP_REAL* Y, KPP_REAL* Z, KPP_REAL* E, int* P, int SaveLU);
void SDIRK_ErrorScale(int N, int ITOL, KPP_REAL AbsTol[], KPP_REAL RelTol[], KPP_REAL Y[],
                      KPP_REAL SCAL[]);
KPP_REAL SDIRK_ErrorNorm(int N, KPP_REAL Y[], KPP_REAL SCAL[]);
int SDIRK_ErrorMsg(int code, KPP_REAL T, KPP_REAL H);
void SDIRK_PrepareMatrix(KPP_REAL H, KPP_REAL T, KPP_REAL Y[], KPP_REAL FJAC[], int SkipJac,
                         int SkipLU, KPP_REAL E[], int IP[], int Reject, int ISING, int ISTATUS[]);
void SDIRK_Solve(char Transp, KPP_REAL H, int N, KPP_REAL E[], int IP[], int ISING, KPP_REAL RHS[],
                 int ISTATUS[]);
void Sdirk4a();
void Sdirk4b();
void Sdirk2a();
...
void INTEGRATE_ADJ(int NADJ, KPP_REAL Y[], KPP_REAL Lambda[][NVAR], KPP_REAL TIN, KPP_REAL TOUT,
    ...
    T1 = TIN;
    T2 = TOUT;
    Ierr = SDIRKADJ(NVAR, NADJ, T1, T2, Y, Lambda, RTOL, ATOL, ATOL_adj, RTOL_adj, RCNTRL, ICNTRL,
                    RSTATUS, ISTATUS);
    ...
    if (Ierr < 0) printf("SDIRK: Unsuccessful exit at T=%f (Ierr=%d)\n", TIN, Ierr);
...
int SDIRKADJ(int N, int NADJ, KPP_REAL Tinitial, KPP_REAL Tfinal, KPP_REAL Y[],
    ...
    Ierr = SDIRK_FwdInt(N, Tinitial, Tfinal, Y, Hmax, Hmin, Hstart, Roundoff, AbsTol, RelTol,
                        ISTATUS, RSTATUS, Max_no_steps, NewtonMaxit, NewtonTol, ThetaMin, FacSafe,
                        FacMin, FacMax, FacRej, Qmin, Qmax, ITOL, SaveLU);

    /*~~~>  Call adjoint integration */
    Ierr = SDIRK_DadjInt(N, NADJ, Lambda, SaveLU, ISTATUS, ITOL, AbsTol_adj, RelTol_adj,
                         NewtonMaxit, ThetaMin, NewtonTol, DirectADJ);
...
int SDIRK_FwdInt(int N, KPP_REAL Tinitial, KPP_REAL Tfinal, KPP_REAL Y[], KPP_REAL Hmax,
    ...
    /*~~~> Local variables: */
    KPP_REAL Z[Smax][NVAR], G[NVAR], TMP[NVAR], NewtonRate, SCAL[NVAR], RHS[NVAR], T, H,
        Theta = 0.0, Hratio, NewtonPredictedErr, Qnewton, Err, Fac, Hnew, Tdirection,
        NewtonIncrement, NewtonIncrementOld = 0.0;
    ...
    if (ABS(H) <= ((KPP_REAL)10.0 * Roundoff)) H = (KPP_REAL)(1.0e-06);
    H = MIN(ABS(H), Hmax);
    H = SIGN(H, Tdirection);
    SkipLU = 0;
    SkipJac = 0;
    ...
    while ((Tfinal - T) * Tdirection - Roundoff > ZERO) { /* Tloop */
        ...
        if (SkipLU == 0) { /* This time around skip the Jac update and LU */
            SDIRK_PrepareMatrix(H, T, Y, FJAC, SkipJac, SkipLU, E, IP, Reject, IER, ISTATUS);
            if (IER != 0) return SDIRK_ErrorMsg(-8, T, H);
        }

        if (ISTATUS[Nstp] > Max_no_steps) return SDIRK_ErrorMsg(-6, T, H);
            ...
            for (NewtonIter = 0; NewtonIter < NewtonMaxit; NewtonIter++) { /*NewtonLoop*/
                ...
                if (NewtonIter == 0) {
                    ...
                    if (Theta < (KPP_REAL)0.99) {
                        ...
                        NewtonPredictedErr =
                            (NewtonIncrement *
                             pow(Theta, (NewtonMaxit - (NewtonIter + 1)) / (ONE - Theta)));
                        if (NewtonPredictedErr >= NewtonTol) {
                            ...
                            Fac = (KPP_REAL)0.8 *
                                  pow(Qnewton, (-ONE / (1 + NewtonMaxit - NewtonIter + 1)));
                            break;
                        }
        ...
        if (CycleTloop == 0) {
            ...
            for (i = 0; i < rkS; i++) {
                if (rkE[i] != ZERO) WAXPY(N, rkE[i], &Z[i][0], 1, TMP, 1);
            }
            ...
            if (Err < ONE) {   /*~~~> Step is accepted */
                ...
                /* Y(:) <-- Y(:) + Sum_j rkD(j)*Z_j(:) */
                for (i = 0; i < rkS; i++) {
                    if (rkD[i] != ZERO) WAXPY(N, rkD[i], &Z[i][0], 1, Y, 1);
                }
...
int SDIRK_DadjInt(int N, int NADJ, KPP_REAL Lambda[][NVAR], int SaveLU, int ISTATUS[], int ITOL,
    ...
    /*~~~> Local variables: */
    KPP_REAL Y[NVAR];
    KPP_REAL Z[Smax][NVAR], U[Smax][NADJ][NVAR], TMP[NVAR], G[NVAR], NewtonRate, SCAL[NVAR],
        DU[NVAR], T, H, Theta, NewtonPredictedErr, NewtonIncrement, NewtonIncrementOld = 0.0;
    ...
    while (stack_ptr > -1) { /* Tloop */
        ...
        if (!SaveLU) {
            SkipJac = FALSE;
            SkipLU = FALSE;
            SDIRK_PrepareMatrix(H, T, Y, Jac, SkipJac, SkipLU, E, IP, Reject, IER, ISTATUS);
            if (IER != 0) return SDIRK_ErrorMsg(-8, T, H);
        }

        for (istage = rkS - 1; istage >= 0; istage--) { /* Stages Loop */
            /*~~~>  Jacobian of the current stage solution */
            for (i = 0; i < N; i++) TMP[i] = Y[i] + Z[istage][i];
            ...
            if (DirectADJ) {
#ifdef FULL_ALGEBRA
                for (i = 0; i < N; i++) {
                    for (j = 0; j < N; j++) E_adj[i][j] = -Jac[i][j];
                }
                for (i = 0; i < N; i++) E_adj[i][i] = E_adj[i][i] + ONE / (H * rkGamma);

                DGETRF(N, N, E_adj, N, IP_adj, IER);
#else
                for (i = 0; i < LU_NONZERO; i++) E_adj[i] = -Jac[i];
                for (i = 0; i < NVAR; i++) {
                    j = LU_DIAG[i];
                ...
                if (IER != 0) {
                    printf(
                        "At stage %d the matrix used in adjoint computation is "
                        "singular\n",
                        istage);
            ...
            for (iadj = 0; iadj < NADJ; iadj++) { /* adj loop */
                ...
                for (i = 0; i < N; i++) G[i] = rkB[istage] * Lambda[iadj][i];
                if (istage + 1 < rkS) {
...
#endif
                for (i = 0; i < N; i++) G[i] = H * TMP[i];

                if (DirectADJ) {
                    SDIRK_Solve('T', H, N, E_adj, IP_adj, IER, G, ISTATUS);
                    for (i = 0; i < N; i++) U[istage][iadj][i] = G[i];
                } else {
                    /*~~~>  Initializations for Newton iteration */
                    ...
                    for (NewtonIter = 0; NewtonIter < NewtonMaxit; NewtonIter++) {
                        /*~~~>   Prepare the loop-dependent part of the right-hand side */
#ifdef FULL_ALGEBRA
                        for (i = 0; i < N; i++) TMP = MATMUL(TRANSPOSE(Jac), U[istage][iadj][i]);
#else
                        for (i = 0; i < N; i++) JacTR_SP_Vec(Jac, &U[istage][iadj][i], TMP);
#endif
                        for (i = 0; i < N; i++)
                        ...
                        if (NewtonIter == 0) {
                            ...
                            if (Theta < (KPP_REAL)0.99) {
                                ...
                                NewtonPredictedErr = NewtonIncrement *
                                                     pow(Theta, (NewtonMaxit - NewtonIter)) /
                                                     (ONE - Theta);
                                /* Non-convergence of Newton: predicted error too large */
                                if (NewtonPredictedErr >= NewtonTol) break; /* Exit NewtonLoop */
                            } else { /* Non-convergence of Newton: Theta too large */
                                break;
                            }
                        }
                        NewtonIncrementOld = NewtonIncrement;
                        /* Update solution */
                        for (i = 0; i < N; i++) U[istage][iadj][i] -= DU[i];
                        ...
                        if ((NewtonIter >= 4) && NewtonDone) break; /* Exit Newton Loop */
                    }
...
#ifdef FULL_ALGEBRA
                        for (i = 0; i < N; i++) E_adj[i][j] = -Jac[i][j];
                        for (j = 0; j < N; j++) E_adj[j][j] += ONE / (H * rkGamma);
                        DGETRF(N, N, E_adj, N, IP_adj, IER);
#else
                        for (i = 0; i < LU_NONZERO; i++) E_adj[i] = -Jac[i];
                        for (i = 0; i < NVAR; i++) {
        ...
        for (istage = 0; istage < rkS; istage++) {
            for (iadj = 0; iadj < NADJ; iadj++) {
                for (i = 0; i < N; i++) Lambda[iadj][i] += U[istage][iadj][i];
            }
        }
    } /* End of Tloop */
...
void SDIRK_FreeBuffers(int Max_no_steps, int SaveLU) {
    ...
    for (i = 0; i < Max_no_steps; i++) free(chk_Y[i]);
    free(chk_Y);

    for (i = 0; i < Max_no_steps; i++) {
        for (j = 0; j < rkS; j++) { free(chk_Z[i][j]); }
        free(chk_Z[i]);
    }
    free(chk_Z);
...
#ifdef FULL_ALGEBRA
        for (i = 0; i < Max_no_steps; i++) {
            for (j = 0; j < rkS; j++) { free(chk_J[i][j]); }
            free(chk_J[i]);
        }
        free(chk_Z);
#else
        for (i = 0; i < Max_no_steps; i++) free(chk_J[i]);
        free(chk_J);
#endif

        for (i = 0; i < Max_no_steps; i++) free(chk_P[i]);
        free(chk_P);
    }
...
void SDIRK_Push(KPP_REAL T, KPP_REAL H, KPP_REAL Y[], KPP_REAL Z[][NVAR], KPP_REAL E[], int P[],
    ...
    for (i = 0; i < NVAR; i++) {
        ...
        for (j = 0; j < rkS; j++) chk_Z[stack_ptr][j][i] = Z[j][i];
    }

    if (SaveLU) {
#ifdef FULL_ALGEBRA
        for (i = 0; i < NVAR; i++) {
            for (j = 0; j < NVAR; j++) chk_J[stack_ptr][i][j] = E[i][j];
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
void SDIRK_Pop(KPP_REAL* T, KPP_REAL* H, KPP_REAL* Y, KPP_REAL* Z, KPP_REAL* E, int* P,
               int SaveLU) {
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    *T = chk_T[stack_ptr];
    for (i = 0; i < NVAR; i++) {
        ...
        for (j = 0; j < rkS; j++) Z[(j * NVAR) + i] = chk_Z[stack_ptr][j][i];
    }
...
void SDIRK_ErrorScale(int N, int ITOL, KPP_REAL AbsTol[], KPP_REAL RelTol[], KPP_REAL Y[],
    ...
    int i;
    if (ITOL == 0) {
        for (i = 0; i < NVAR; i++) SCAL[i] = ONE / (AbsTol[0] + RelTol[0] * ABS(Y[i]));
    } else {
        for (i = 0; i < NVAR; i++) SCAL[i] = ONE / (AbsTol[i] + RelTol[i] * ABS(Y[i]));
    }
...
KPP_REAL SDIRK_ErrorNorm(int N, KPP_REAL Y[], KPP_REAL SCAL[]) {
    ...
    for (i = 0; i < N; i++) Err = Err + pow((Y[i] * SCAL[i]), 2);
...
#ifdef FULL_ALGEBRA
        for (j = 0; j < NVAR; j++) {
            for (i = 0; i < NVAR; i++) E[j][i] = -FJAC[j][i];
            E[j][j] = E[j][j] + HGammaInv;
        }
        DGETRF(NVAR, NVAR, E, NVAR, IP, ISING);
#else
        for (i = 0; i < LU_NONZERO; i++) E[i] = -FJAC[i];
        for (i = 0; i < NVAR; i++) {
            j = LU_DIAG[i];
            E[j] = E[j] + HGammaInv;
        }
...
void JAC_CHEM(KPP_REAL T, KPP_REAL Y[], KPP_REAL JV[]) {
    ...
    for (j = 0; j < NVAR; j++) {
        for (i = 0; i < NVAR; i++) JV[j][i] = (KPP_REAL)0.0;
    } /* end for */

    for (i = 0; i < LU_NONZERO; i++) JV[LU_ICOL[i]][LU_IROW[i]] = JS[i];
#else
