...
int S2A = 1, S2B = 2, S3A = 3, S4A = 4, S4B = 5;

int sdMethod, rkS;

KPP_REAL rkGamma, rkA[Smax][Smax], rkB[Smax], rkELO, rkBhat[Smax], rkC[Smax], rkD[Smax], rkE[Smax],
    rkTheta[Smax][Smax], rkAlpha[Smax][Smax];
...
int SDIRK(int N, KPP_REAL Tinitial, KPP_REAL Tfinal, KPP_REAL Y[], KPP_REAL RelTol[],
          KPP_REAL AbsTol[], KPP_REAL RCNTRL[], int ICNTRL[], KPP_REAL RSTATUS[], int ISTATUS[]);
int SDIRK_Integrator(int N, KPP_REAL Tinitial, KPP_REAL Tfinal, KPP_REAL Y[], int Ierr,
                     KPP_REAL Hstart, KPP_REAL Hmin, KPP_REAL Hmax, KPP_REAL Roundoff,
...
void SDIRK_Solve(KPP_REAL H, int N, KPP_REAL E[], int IP[], int ISING, KPP_REAL RHS[],
                 int ISTATUS[]);
void Sdirk4a(void);
void Sdirk4b(void);
void Sdirk2a(void);
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    T1 = TIN;
    T2 = TOUT;
    Ierr = SDIRK(NVAR, T1, T2, VAR, RTOL, ATOL, RCNTRL, ICNTRL, RSTATUS, ISTATUS);

    /*~~~> Debug option: print number of steps
       Ntotal += ISTATUS[Nstp]; */

    if (Ierr < 0) { printf("SDIRK: Unsuccessful exit at T=%f(Ierr=%d)", TIN, Ierr); }
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    KPP_REAL Hmin = 0, Hmax = 0, Hstart = 0, Roundoff, FacMin = 0, FacMax = 0, FacSafe = 0,
             FacRej = 0, ThetaMin, NewtonTol, Qmin, Qmax;

    int ITOL, NewtonMaxit, i, Ierr = 0;
    ...
    if (Ierr < 0) { return Ierr; } /*end if */

    Ierr = SDIRK_Integrator(N, Tinitial, Tfinal, Y, Ierr, Hstart, Hmin, Hmax, Roundoff, AbsTol,
                            RelTol, ISTATUS, RSTATUS, ITOL, Max_no_steps, StartNewton, NewtonTol,
                            ThetaMin, FacSafe, FacMin, FacMax, FacRej, Qmin, Qmax, NewtonMaxit);
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    KPP_REAL Z[Smax][NVAR], G[NVAR], TMP[NVAR], NewtonRate, SCAL[NVAR], RHS[NVAR], T, H,
        Theta = 0, Hratio, NewtonPredictedErr, Qnewton, Err = 0, Fac, Hnew, Tdirection,
        NewtonIncrement = 0, NewtonIncrementOld = 0;

    int IER = 0, istage, NewtonIter, IP[NVAR], Reject, FirstStep, SkipJac, SkipLU, NewtonDone,
        CycleTloop, i, j;
    ...
    while ((Tfinal - T) * Tdirection - Roundoff > ZERO) { /* Tloop */
        ...
        if (CycleTloop == 0) {
            ...
            for (i = 0; i < rkS; i++) {
                if (rkE[i] != ZERO) { WAXPY(N, rkE[i], &Z[i][0], 1, TMP, 1); } /* end if */
            } /* end for */
            ...
            if (Err < ONE) {   /*~~~> Step is accepted */
                ...
                /* Y(:) <-- Y(:) + Sum_j rkD(j)*Z_j(:) */
                for (i = 0; i < rkS; i++) {
                    if (rkD[i] != ZERO) { WAXPY(N, rkD[i], &Z[i][0], 1, Y, 1); } /* end if */
                } /* end for */
                ...
                if (Reject == 1) { Hnew = Tdirection * MIN(ABS(Hnew), ABS(H)); } /* end if */
                ...
                if ((T + Hnew / Qmin - Tfinal) * Tdirection > ZERO) {
                    ...
                    if (SkipLU == 0) { H = Hnew; } /* end internal if */
                } /* end if else */
                ...
                if (ISTATUS[Nacc] >= 1) { ISTATUS[Nrej]++; } /* end if */
            } /* end if else */
        } /* end CycleTloop if */
    } /* end Tloop */
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    for (i = 0; i < N; i++) { Err = Err + pow((Y[i] * SCAL[i]), 2); } /* end for */
 ...
 *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    while (ISING != 0) {
...
#ifdef FULL_ALGEBRA
        for (j = 0; j < NVAR; j++) {
            for (i = 0; i < NVAR; i++) { E[j][i] = -FJAC[j][i]; } /* end for */
...
#else
        for (i = 0; i < LU_NONZERO; i++) { E[i] = -FJAC[i]; } /* end for */
        ...
        if (ISING != 0) {
            ...
            if (ConsecutiveSng >= 6) { return; /* Failure */ } /* end internal if */
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{
    ...
    for (j = 0; j < NVAR; j++) {
        for (i = 0; i < NVAR; i++) { JV[j][i] = (KPP_REAL)0.0; } /* end for */
    } /* end for */

    for (i = 0; i < LU_NONZERO; i++) { JV[LU_ICOL[i]][LU_IROW[i]] = JS[i]; } /* end for */
#else
