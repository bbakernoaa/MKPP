...
def run_legacy_kpp(cells=100, steps=1440):
    ...
    print(f"   [Legacy KPP] Launching saprc99_diurnal.exe ({cells} cells x {steps} timesteps)...", flush=True)
    start = time.perf_counter()
    res = subprocess.run([exe_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=kpp_dir)
    elapsed_ms = (time.perf_counter() - start) * 1000.0
    print(f"   [Legacy KPP] Completed in {elapsed_ms/1000.0:.2f} s ({elapsed_ms:.1f} ms)", flush=True)
    return elapsed_ms

def run_mkpp_runner(cells=1000, steps=1440, threads=8, label="MKPP Runner", execution_mode="kokkos"):
    env = os.environ.copy()
    ...
    cmd = [cmd_path]
    print(f"   [{label}] Launching e2e_saprc99_runner (cells={cells}, steps={steps}, threads={threads}, mode={execution_mode})...", flush=True)
    start = time.perf_counter()
    ...
    for line in res.stdout.split('\n'):
        if "Time:" in line:
            try:
                ...
                print(f"   [{label}] Kernel execution reported: {kernel_ms:.1f} ms (Total wall: {elapsed_ms:.1f} ms)", flush=True)
                return kernel_ms
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    mkpp_ms = run_mkpp_runner(cells=100, steps=1440, threads=1, label="MKPP 100 cells", execution_mode="serial")

    print(f"\nLegacy KPP (Sparse Fortran 90, 79 sp) : {kpp_ms/1000.0:8.2f} s ({kpp_ms:10.1f} ms)", flush=True)
    print(f"MKPP Full (AOT C++ Kokkos, 79 sp)     : {mkpp_ms/1000.0:8.2f} s ({mkpp_ms:10.1f} ms) -> {kpp_ms/mkpp_ms:5.2f}x Speedup", flush=True)
    ...
    for c in cell_counts:
        t_ms = run_mkpp_runner(cells=c, steps=scaling_steps, threads=1, label=f"Scaling {c} cells", execution_mode="serial")
    ...
    print(f"{'Threads':>10} | {'Time (ms)':>15} | {'Speedup':>12} | {'Efficiency (%)':>18}", flush=True)
    print("-" * 60, flush=True)
    t1_time = None
    for threads in [1, 2, 4, 8, 16]:
        t_ms = run_mkpp_runner(cells=10000, steps=100, threads=threads, label=f"OpenMP {threads} threads", execution_mode="kokkos")
        if threads == 1:
            t1_time = t_ms
            speedup = 1.0
            eff = 100.0
