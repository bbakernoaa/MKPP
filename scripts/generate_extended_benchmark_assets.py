...
def main() -> None:
    ...
    manifest = {
        "schema_version": 1,
        "mechanism_id": "saprc99",
        "version": "1.0.0",
        "canonical_source": [{"path": "mechanisms/openatmos/saprc99/mechanism.json", "revision": "openatmos-v1", "sha256": sha}],
        "canonical_species": [
            {"id": name, "role": "fixed" if name in fixed else "active", "phase": "gas", "unit": "mol m-3", "scale": 1.0}
            for name in names
        ],
        ...
        "bindings": {
            solver: {
                "revision": "workspace",
                "assets": ["mechanisms/openatmos/saprc99/mechanism.json"],
                "species_map": {name: name for name in names},
                "photolysis_map": {name: name for name in photolysis},
            }
    ...
    forcing = {
        name: {
            "unit": "s-1",
            "points": [
                {"time_seconds": 0.0, "value": 1.0e-6},
                {"time_seconds": 300.0, "value": 1.05e-6},
                {"time_seconds": 600.0, "value": 1.0e-6},
            ],
        }
        for name in photolysis
    }
    base = {
        "schema_version": 1,
        "version": "1.0.0",
        "mechanism": {"id": "saprc99", "version": "1.0.0", "sha256": sha},
        "initial_state": initial,
        "environment": {"temperature": {"value": 298.15, "unit": "K"}, "pressure": {"value": 101325.0, "unit": "Pa"}},
        "forcing": forcing,
        "integration": {
            "start_seconds": 0.0,
            "horizon_seconds": 600.0,
            "checkpoints_seconds": [300.0, 600.0],
            "forcing_boundaries_seconds": [0.0, 300.0, 600.0],
        },
        "accuracy": {
            "rtol": 1.0e-6,
            "atol_by_species": {name: 1.0e-15 for name in names},
    ...
    for role, cells, seed in (("calibration", [1, 10], 1701), ("measurement", [1, 10, 100], 2718)):
        ...
        (ROOT / "benchmarks/solver_comparison/scenarios" / f"saprc99-{role}-v1.yaml").write_text(
