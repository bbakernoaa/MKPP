...
def benchmark_build_metrics(mech_path, lump_path=None):
    ...
    for solver_name, tableau in SOLVER_COEFFICIENTS.items():
        ...
        print(
            f"{solver_name.upper():<10} | {tableau.stages:<8} | {int(tableau.ELO):<10} | {header_size_kb:>8.1f} KB | {t_gen:>11.2f} ms"
        )
        results[solver_name] = {
...
def run_cpp_solver_benchmark(solvers):
    ...
    print(
        f"{'Solver':<10} | {'Build Status':<12} | {'1 Cell (ms)':<12} | {'1000 Cells (ms)':<16} | {'Throughput (cell-steps/s)':<24}"
    )
    print("-" * 80)
