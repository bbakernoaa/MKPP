# Research Notes: Exaero C++ Utility

**Objective**: Resolve technical unknowns required to implement the Exaero Utility.

## Needs Clarification Resolutions

**Unknown**: How to elegantly link the MKPP generated mechanisms into the C++ utility?
...
**Decision**: Allocate persistent `Kokkos::View<int*, Kokkos::DeviceSpace>` index maps during the initial utility instantiation. During each timestep execution, `Kokkos::sort` or a custom histogram sort will simply reorder the values inside the persistent view. The `TeamPolicy` will iterate over the sorted indices rather than the physical grid directly.
