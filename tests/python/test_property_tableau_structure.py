...
def test_property_2_tableau_structural_consistency(solver_name: str):
    ...
    **Validates: Requirements 2.1, 2.2**
    ...
    assert isinstance(tableau, RosenbrockTableau), (
    ...
    assert isinstance(stages, int), (
        f"{solver_name}: stages must be an int, got {type(stages)}"
    )
    assert stages > 0, (
        f"{solver_name}: stages must be positive, got {stages}"
    )

    # Strictly lower-triangular arrays: len == stages*(stages-1)//2
    expected_tri = stages * (stages - 1) // 2
    assert len(tableau.A) == expected_tri, (
        f"{solver_name}: len(A)={len(tableau.A)} != stages*(stages-1)//2={expected_tri}"
    )
    assert len(tableau.C) == expected_tri, (
        f"{solver_name}: len(C)={len(tableau.C)} != stages*(stages-1)//2={expected_tri}"
    )

    # Stage-length arrays: len == stages
    assert len(tableau.M) == stages, (
        f"{solver_name}: len(M)={len(tableau.M)} != stages={stages}"
    )
    assert len(tableau.E) == stages, (
        f"{solver_name}: len(E)={len(tableau.E)} != stages={stages}"
    )
    assert len(tableau.Alpha) == stages, (
        f"{solver_name}: len(Alpha)={len(tableau.Alpha)} != stages={stages}"
    )
    assert len(tableau.Gamma) == stages, (
        f"{solver_name}: len(Gamma)={len(tableau.Gamma)} != stages={stages}"
    )
    assert len(tableau.NewF) == stages, (
        f"{solver_name}: len(NewF)={len(tableau.NewF)} != stages={stages}"
    )

    # ELO must be a positive float
    assert isinstance(tableau.ELO, float), (
        f"{solver_name}: ELO must be a float, got {type(tableau.ELO)}"
    )
    assert tableau.ELO > 0.0, (
        f"{solver_name}: ELO must be positive, got {tableau.ELO}"
    )
