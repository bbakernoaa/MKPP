...
def test_campaign_execution_fails_without_silent_or_partial_output(tmp_path: Path, capsys: object) -> None:
    ...
    assert (
        main(
            ["--campaign", "benchmarks/solver_comparison/campaigns/cpu-primary-v1.yaml", "--output", str(output)]
        )
        == EXIT_ENVIRONMENT
    )
...
def test_campaign_execution_retains_only_real_named_runner_results(tmp_path: Path, monkeypatch) -> None:
    ...
    def completed(command, **_):
        ...
        sha = "0" * 64
        result = {
            "schema_version": 1, "campaign_id": "test-ts1", "run_id": solver, "block": 0, "order_position": 0,
            ...
            "timing": {
                "boundary": "steady_state_solve", "elapsed_ms": 1.0, "cell_steps": 1,
                "clock": "steady", "synchronized": True, "synchronization": "fence", "lifecycle_ms": {},
            },
            "state": {"checkpoints": [{"time_seconds": 1.0, "values": {"O3": 1.0}}]},
            "work": {"rhs": {"available": True, "value": 1}}, "status": "success", "diagnostics": [],
        }
