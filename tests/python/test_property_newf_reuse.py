...
def _build_mechanism_with_lu_plan(n_species: int = 2) -> MechanismDefinition:
    ...
    """
    species = [
        SpeciesDefinition(name=f"sp_{i}", phase=PhaseMode.GAS)
    ...
    plan = SymbolicLUPlan(
        ...
        backward_sub_steps=[
            (i, f"y_{i} / U_{i}_{i}") for i in reversed(range(n_species))
        ],
    )
...
def test_property_5_newf_false_no_fresh_evaluation(
    ...
    **Validates: Requirements 3.2**
    ...
    fresh_f_pattern = re.compile(
        rf'\bdouble\s+F{reuse_stage}_\d+\s*='
    )
...
def test_property_5_newf_false_reuse_comment(
    ...
    **Validates: Requirements 3.2**
    ...
    assert reuse_comment_pattern.search(code) is not None, (
...
def test_property_5_newf_false_rhs_references_prior_f(
    ...
    **Validates: Requirements 3.2**
    ...
    assert len(rhs_lines) > 0, (
        f"{solver_name}: No rhs{reuse_stage} lines found in generated code"
    )

    # At least one rhs line should reference the prior F variable
    has_prior_f_ref = any(
        f'F{prior_f_stage}_' in line for line in rhs_lines
    )
    ...
    has_reuse_stage_f_ref = any(
        f'F{reuse_stage}_' in line for line in rhs_lines
    )
...
def test_property_5_negative_control_all_newf_true(solver_name: str):
    ...
    **Validates: Requirements 3.2**
    ...
    for stage in range(1, tableau.stages + 1):
        ...
        assert fresh_f_pattern.search(code) is not None, (
