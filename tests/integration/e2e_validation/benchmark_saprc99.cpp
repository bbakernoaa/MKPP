...
template <typename SolverKernelsType>
double run_diurnal_benchmark(int num_cells, int num_species, int num_steps = 1440, double dt = 60.0) {
    using ViewType = Kokkos::View<double**, Kokkos::LayoutLeft, typename ExecSpace::memory_space>;
...
int main(int argc, char** argv) {
    Kokkos::initialize(argc, argv);
    {
        ...
        std::cout << std::setw(12) << "Grid Cells"
                  << std::setw(18) << "Time (ms)"
                  << std::setw(22) << "Throughput (cells/s)" << "\n";
        std::cout << "---------------------------------------------------------\n";
        ...
        for (int n_cells : cell_counts) {
            ...
            std::cout << std::setw(12) << n_cells
                      << std::setw(18) << std::fixed << std::setprecision(2) << time_ms
                      << std::setw(22) << std::scientific << std::setprecision(2) << throughput << "\n";
        }
    }
    Kokkos::finalize();
    return 0;
